                                     ...
                                     Tensor *result)
{
  try
  {
    ...
    while (data.size() < tensors_size)
      data.push_back(**tensors++);
   ...
   const char *Add(Tensor a, Tensor other, float alpha, Tensor *result)
{
  ...
  {
    auto dtype1 = a->dtype();
    auto dtype2 = other->dtype();
    if (dtype1 == torch::kInt32 && dtype2 == torch::kInt32)
    {
      int alphaInt = (int)round(alpha);
      *result = new at::Tensor(a->add(*other, alphaInt));
    }
    else
    {
   ...
   const char *Add_(Tensor a, Tensor other, float alpha, Tensor *result)
{
  try
  {
    ...
    {
      int alphaInt = (int)round(alpha);
      *result = new at::Tensor(a->add_(*other, alphaInt));
    }
    else
    {
   ...
   const char *Sub(Tensor a, Tensor other, float alpha, Tensor *result)
{
  try
  {
    ...
    {
      int alphaInt = (int)round(alpha);
      *result = new at::Tensor(a->sub(*other, alphaInt));
    }
    else
    {
      *result = new at::Tensor(a->sub(*other, alpha));
    }
    return nullptr;
  }
  catch (const c10::Error &e)
   ...
   const char *Sub_(Tensor a, Tensor other, float alpha, Tensor *result)
{
  try
  {
    ...
    {
      int alphaInt = (int)round(alpha);
      *result = new at::Tensor(a->sub_(*other, alphaInt));
    }
    else
    {
